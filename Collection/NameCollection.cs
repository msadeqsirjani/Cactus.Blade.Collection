...
namespace Cactus.Blade.Collection
{
    /// <summary>
    /// Represents a collection that can be retrieved by name.
    /// </summary>
    /// <typeparam name="T">The type of items in the collection.</typeparam>
    public class NameCollection<T> : IReadOnlyCollection<T>, IReadOnlyDictionary<string, T>
    {
        private const string DefaultDefaultName = "default";

        private readonly IReadOnlyDictionary<string, T> _valuesByName;
        private readonly Func<T, string> _dictionaryKeySelector;

        /// <summary>
        /// Initializes a new instance of the <see cref="NameCollection{T}"/> class
        ...
        /// </param>
        public NameCollection(IEnumerable<T> values, Func<T, string> getName,
            IEqualityComparer<string> stringComparer = null, string defaultName = DefaultDefaultName, bool strict = true)
        {
            Guard.Guard.Against.Null(values, nameof(values));
            Guard.Guard.Against.Null(getName, nameof(getName));

            StringComparer = stringComparer ?? OrdinalIgnoreCase;
            DefaultName = defaultName.IsNullOrEmpty() ? defaultName : DefaultDefaultName;
        ...
        /// </summary>
        /// <param name="name">
        /// The name to locate in the <see cref="NameCollection{T}" />.
        /// </param>
        /// <returns>
        ...
        /// </param>
        /// <returns>
        /// <see langword="true" /> if the <see cref="NameCollection{T}" /> contains an value with the
        ...
        {
            return name.IsNullOrEmpty() || StringComparer.Equals(name, DefaultName);
        }
        ...
        public IEnumerable<string> Keys => this.Select(_dictionaryKeySelector);
        ...
        {
            return new KeyNotFoundException(IsDefaultName(name)
